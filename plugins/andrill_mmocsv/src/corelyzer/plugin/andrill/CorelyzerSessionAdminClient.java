...
public class CorelyzerSessionAdminClient extends Thread{
    ...
    //==========================================
    public CorelyzerSessionAdminClient(String s) {
        this.server = s;
        username = null;
        password = null;
    ...
    public static void main(String[] args) {
        try {
            CorelyzerSessionAdminClient c = 
                new CorelyzerSessionAdminClient(args[0]);
            c.start();
            c.join();
        }
        catch( Exception e ) {
    ...
    private boolean processNewSplitCore(String[] toks) {
        if( toks.length < 4 )
        {
            ...
            System.out.println("Usage: new-split-core <image url> " +
                               " <section name> <image dpi_x> [<image dpi_y>]");
            System.out.println("Please try again.");
            return false;
        }

        float dpi_x, dpi_y, length;

        try {
            dpi_x = Float.parseFloat(toks[3]);

            if( toks.length == 5 ) {
                dpi_y = Float.parseFloat(toks[4]);
            } else {
                dpi_y = dpi_x;
            }
        }
