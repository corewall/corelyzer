...
public class DeleteSectionAction extends AbstractAction {

	public DeleteSectionAction(final String[] toks) {
		super(toks);
	}
	...
	public void run() {
		...
		if (cmds.length == 7) { // using leg-site-hole-core-section naming
			...
			System.out.println("---> [DeleteSection] Section URL is: " + url);
			...
			System.err.println("---> [DeleteSection] Some command " + "length that I do not understand");

			return;
		}

		if (location != null) {
			CRExperimentController.deleteSection(location);
		} else {
			System.out.println("---> [DeleteSection] The section is not loaded.");
