...
public class ROMEUtils {
	 ...
	 */
	public static SyndCategory createCategory(final String name, final String taxonomy) {
	 ...
	 * @return the link.
	 */
	public static SyndLink createLink(final String href, final String rel, final String title, final String type) {
	 ...
	 */
	public static SyndFeed readFeed(final String url) {
		SyndFeedInput input = new SyndFeedInput();
		try {
			SyndFeed feed = input.build(new InputStreamReader(new URL(url).openStream()));

			if (feed.getLink() == null) {
				// NOTE: it looks like ROME doesn't currently parse feed links
				// that are
				// rel="self" or if there are multiple links for Atom 1.0.
				// This has been reported
				// (https://rome.dev.java.net/issues/show_bug.cgi?id=64)
				// and hopefully will be fixed in the next version.
				feed.setLink(url);
			}

			// System.out.println("---> [DEBUG] ROMEUtils' url is: " +
			// feed.getLink());
			return feed;
		} catch (IllegalArgumentException e) {
			System.err.println("IllegalArgumentException when parsing feed '" + url + "'");
		} catch (MalformedURLException e) {
			System.err.println("MalformedURLException when parsing feed '" + url + "'");
		} catch (FeedException e) {
			System.err.println("FeedException when parsing feed '" + url + "'");
		} catch (IOException e) {
	 ...
	 *            the filename.
	 */
	public static void writeFeed(final SyndFeed feed, final String filename) {
		Writer writer = null;
		try {
