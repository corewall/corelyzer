...
public class ServerSubscribeController {
	public static void processSubscribe(final SharingClientConnectionThread clientThread) {
		...
		System.out.println("---> [SSA] Process client 'Subscribe' request");
		...
		try {
			...
			String feedURL = server.getUrlPrefix() + "/" + authorName + "/" + projectName + "/feed.xml";
			dos.writeUTF(feedURL);

			dos.write(SharingServerResponse.SUBSCRIBE_SUCCESS);
			dos.flush();
		} catch (IOException e) {
			System.err.println("---> [SSA] Exception in ServerSubscribeController: " + e);
			e.printStackTrace();
		}
	}
}
