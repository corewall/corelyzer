 ...
 * Lima for sending in the bug report. Released as 1.0.20.</li>
...
public class ImageInfo {
	...
	private static void printCompact(final String sourceName, final ImageInfo imageInfo) {
		...
		System.out.println(sourceName + SEP + imageInfo.getFormatName() + SEP + imageInfo.getMimeType() + SEP + imageInfo.getWidth() + SEP
				+ imageInfo.getHeight() + SEP + imageInfo.getBitsPerPixel() + SEP + imageInfo.getNumberOfImages() + SEP + imageInfo.getPhysicalWidthDpi() + SEP
				+ imageInfo.getPhysicalHeightDpi() + SEP + imageInfo.getPhysicalWidthInch() + SEP + imageInfo.getPhysicalHeightInch() + SEP
				+ imageInfo.isProgressive());
	}

	private static void printLine(final int indentLevels, final String text, final float value, final float minValidValue) {
	...
	private static void run(final String sourceName, final InputStream in, final ImageInfo imageInfo, final boolean verbose) {
